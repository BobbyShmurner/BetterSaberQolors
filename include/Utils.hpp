...
namespace Utils {
	...
	void LogHierarchy(UnityEngine::Transform* trans, int level = 0) {
		if (level == 0) getLogger().info("Logging Hierarchy for %s:", Utils::Il2cppStrToStr(trans->get_name()).c_str());
		...
		for (int i = 0; i < trans->get_childCount(); i++) {
			getLogger().info("%s[%i] %s", start.c_str(), i, Utils::Il2cppStrToStr(trans->GetChild(i)->get_name()).c_str());
